...
> **Structure note.** This file was generated by `create-medusa-app`, which
> scaffolds its own Turborepo. That nested wrapper has been removed: the Medusa
> app now lives at `apps/backend` inside the House of Sirka monorepo, alongside
> `apps/storefront` and `packages/shared`, with a single `turbo.json` and one
> lockfile at the repository root. Paths below that assume `apps/backend/...`
> relative to a separate backend root should be read relative to the repo root
> instead. See `docs/adr/002-monorepo-structure.md`.
