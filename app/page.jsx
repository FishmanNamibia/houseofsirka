...
const CUSTOMER_KEY = "house-of-sirka-customer";
...
const permissions = {
  "Super Admin": ["dashboard", "products", "content", "orders", "promotions", "inventory", "reports"],
  Admin: ["dashboard", "products", "content", "orders", "promotions", "inventory", "reports"],
  "Content Manager": ["dashboard", "content", "reports"],
  "Order Manager": ["dashboard", "orders", "reports"],
  "Inventory Manager": ["dashboard", "inventory", "reports"],
  "Marketing Manager": ["dashboard", "content", "promotions", "reports"],
};
...
const seedProducts = [
  ...
  },
  {
    ...
    description:
      "Single-breasted blazer with a structured shoulder, satin lining, and office-to-evening finish.",
  ...
  },
  {
    ...
    description:
      "Breathable linen-blend shirt and relaxed trouser set for polished warm-weather styling.",
    tags: ["New arrival"],
    rating: 4.6,
...
export default function Home() {
  ...
  function canOpen(tabId) {
    return activePermissions.includes(tabId);
  }
  ...
  function updateStock(productId, variantId, stock) {
    ...
    updateProduct(productId, (item) => ({
      ...
      variants: item.variants.map((row) =>
        row.id === variantId ? { ...row, stock: Number(stock || 0) } : row,
      ),
    }));
    setStore((current) => ({
      ...current,
      inventoryLogs: [
        {
          ...
          change: `${variant?.stock || 0} to ${stock}`,
  ...
  return (
    <main className="min-h-screen bg-paper text-ink">
      <div className="bg-garden px-4 py-2 text-center text-sm font-semibold text-white">
        {store.content.announcement}
      </div>

      <header className="sticky top-0 z-30 border-b border-brass/25 bg-pearl/88 px-4 backdrop-blur-xl md:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 py-4">
          ...
          <nav className="flex flex-wrap items-center gap-1 text-sm font-semibold">
            {[
              ...
              <a
                key={item.label}
                href={item.href}
                className="rounded-md px-3 py-2 text-charcoal transition hover:bg-petal hover:text-wine"
              >
                {item.label}
              </a>
            ))}
          ...
          <button
            ...
            className="inline-flex h-11 items-center gap-2 rounded-md border border-wine bg-wine px-4 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-merlot"
          >
            <ShoppingBag size={18} />
            Cart
            <span className="grid min-w-6 place-items-center rounded-full bg-marigold px-2 py-0.5 text-xs text-ink">
              {cartCount}
            </span>
          </button>
        </div>
      ...
      <section id="shop" className="floral-paper relative overflow-hidden border-b border-brass/25">
        <div className="mx-auto grid min-h-[calc(100vh-116px)] max-w-7xl gap-10 px-4 py-12 md:px-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:py-16">
          <div className="relative z-10">
            <div className="mb-6 inline-flex items-center gap-3 rounded-full border border-brass/35 bg-pearl/80 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-garden shadow-sm">
              <span className="h-2 w-2 rounded-full bg-marigold" />
              {store.content.heroBadge}
            </div>
            <h1 className="sr-only">{store.content.heroTitle}</h1>
            <div className="w-fit max-w-full border border-brass/30 bg-pearl/82 p-4 shadow-sm backdrop-blur">
              <img
                src="/house-of-sirka-logo-final.png"
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-charcoal/72 md:text-xl">
              {store.content.heroSubtitle}
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#catalog"
                className="inline-flex h-12 items-center gap-2 rounded-md bg-wine px-5 font-bold text-white shadow-sm transition hover:bg-merlot"
              >
                {store.content.heroCtaPrimary} <ChevronRight size={18} />
              </a>
              <a
                href="#collections"
                className="inline-flex h-12 items-center gap-2 rounded-md border border-brass/45 bg-pearl px-5 font-bold text-garden transition hover:border-wine hover:text-wine"
              >
            ...
            <div className="mt-10 grid max-w-2xl gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="border border-brass/25 bg-pearl/82 p-4 shadow-sm">
                  <span className="text-xs font-bold uppercase text-charcoal/55">{label}</span>
                  <strong className="mt-1 block font-display text-3xl text-wine">{value}</strong>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative min-h-[560px]">
            <div className="absolute left-0 top-8 hidden w-48 border border-brass/30 bg-pearl p-4 shadow-soft md:block">
              <p className="font-display text-2xl text-merlot">{store.content.fittingNoteTitle}</p>
              <p className="mt-2 text-sm leading-6 text-charcoal/65">
                {store.content.fittingNoteText}
              </p>
            </div>
            <div className="absolute right-0 top-0 h-[82%] w-[78%] border border-brass/40 bg-cream p-3 shadow-soft">
              <img
            ...
            <div className="absolute bottom-0 left-8 w-[58%] border border-brass/35 bg-pearl p-3 shadow-soft">
              <img
              ...
              <div className="flex items-center justify-between gap-3 px-2 py-3">
                <span className="font-display text-2xl text-merlot">{store.content.heroSecondaryLabel}</span>
                <span className="rounded-full bg-marigold px-3 py-1 text-xs font-black text-ink">
      ...
      <div className="lace-edge h-3 bg-pearl text-brass/45" aria-hidden="true" />

      <section id="collections" className="border-b border-brass/25 bg-pearl px-4 py-12 md:px-8 md:py-16">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            {[
              ...
              <button
                ...
                className="group relative min-h-72 overflow-hidden rounded-md border border-brass/25 bg-cream text-left shadow-sm"
              >
                <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-300 group-hover:scale-105" />
                <span className="absolute inset-0 bg-gradient-to-t from-merlot/74 via-merlot/10 to-transparent" />
                <span className="absolute bottom-0 block p-5 text-white">
                  <span className="block font-display text-3xl">{title}</span>
                  <span className="mt-1 block text-sm font-bold uppercase tracking-[0.16em] text-blush">
                    {category}
                  </span>
                </span>
              </button>
      ...
      <section id="catalog" className="bg-paper px-4 py-12 md:px-8 md:py-16">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-3 rounded-md border border-brass/25 bg-pearl p-4 shadow-sm lg:grid-cols-[1.3fr_repeat(5,1fr)]">
            ...
            <label className="grid gap-2 text-sm font-bold text-charcoal/70">
              ...
              />
              <span className="text-xs text-charcoal/55">{fmt(filters.maxPrice)}</span>
            </label>
            <SelectFilter
              label="Sort"
              value={filters.sort}
          ...
          <div className="mt-5 text-sm font-semibold text-charcoal/60">
      ...
      <section id="account" className="bg-petal px-4 py-12 md:px-8 md:py-16">
        <div className="mx-auto max-w-7xl">
          <SectionTitle
          ...
          {!customerEmail ? (
            <div className="mt-8 mx-auto max-w-md">
              <div className="rounded-md border border-brass/25 bg-pearl p-6 shadow-sm">
                <LogIn className="mb-4 text-wine" size={24} />
                <h3 className="font-display text-2xl text-merlot">Customer login</h3>
                ...
                >
                  ...
                  <button type="submit" className="h-11 rounded-md bg-wine font-bold text-white transition hover:bg-merlot">
                    View my orders
                  </button>
                </form>
      ...
      <footer className="bg-merlot px-4 py-8 text-white md:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div>
            <div className="mb-3 w-56 bg-pearl p-2">
...
function SectionTitle({ eyebrow, title, copy }) {
  return (
    <div>
      <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-clay">{eyebrow}</p>
      <h2 className="font-display text-4xl leading-tight text-merlot md:text-5xl">{title}</h2>
      {copy && <p className="mt-3 max-w-2xl text-charcoal/65">{copy}</p>}
    </div>
  );
}
...
function ProductCard({ product, wished, fmt, onView, onEnlarge, onWish, onQuickAdd }) {
  ...
  return (
    <article className="group overflow-hidden rounded-md border border-brass/25 bg-pearl shadow-sm transition hover:-translate-y-1 hover:border-brass/60 hover:shadow-soft">
      <div className="relative aspect-[4/5] bg-cream p-3">
        ...
        <span
          className={classNames(
            "absolute left-5 top-5 rounded-full border border-brass/25 bg-pearl/94 px-3 py-1 text-xs font-black shadow-sm",
            stock <= 2 ? "text-wine" : "text-garden",
          )}
        >
        ...
        <button
          type="button"
          onClick={onWish}
          className="absolute right-5 top-5 grid h-10 w-10 place-items-center rounded-full border border-brass/25 bg-pearl/94 text-wine shadow-sm"
        ...
        <button
          type="button"
          onClick={onEnlarge}
          className="absolute bottom-5 right-5 inline-flex h-10 items-center gap-2 rounded-full border border-brass/25 bg-pearl/94 px-3 text-xs font-black text-garden shadow-sm transition hover:text-wine"
      ...
      <div className="grid gap-3 p-4">
        <div className="flex justify-between gap-3 text-xs font-bold uppercase tracking-[0.16em] text-garden/65">
          <span>{product.category}</span>
          <span>{product.sku}</span>
        </div>
        <div>
          <h3 className="font-display text-2xl leading-tight text-merlot">{product.name}</h3>
          <p className="mt-1 line-clamp-2 text-sm text-charcoal/65">{product.description}</p>
        </div>
          ...
          <div className="flex gap-1">
            {colors.map((color) => (
              <span
                key={color}
                title={color}
                className="h-5 w-5 rounded-full border border-brass/35"
                style={{ backgroundColor: swatches[color] || "#ddd" }}
              />
            ))}
          </div>
          <span className="text-sm font-bold text-charcoal/60">{product.rating}/5</span>
        </div>
        <div className="flex items-baseline gap-2">
          <strong className="text-lg">{fmt(productPrice(product))}</strong>
          {product.salePrice && <del className="text-sm text-charcoal/45">{fmt(product.price)}</del>}
        </div>
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={onView}
            className="h-11 rounded-md bg-wine px-3 text-sm font-bold text-white transition hover:bg-merlot"
          >
            View
          </button>
          <button
            ...
            disabled={!stock}
            onClick={onQuickAdd}
            className="h-11 rounded-md border border-brass/35 bg-paper px-3 text-sm font-bold text-garden transition hover:border-wine hover:text-wine"
          >
            Quick add
          </button>
        </div>
...
function ProductModal({
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="grid max-h-[calc(100vh-2rem)] w-[min(1000px,calc(100vw-2rem))] overflow-auto rounded-md border border-brass/30 bg-pearl shadow-soft lg:grid-cols-[.9fr_1fr]">
        <div className="relative min-h-96 bg-cream p-3 lg:min-h-[620px]">
          <button
            type="button"
            onClick={onEnlarge}
          ...
          <button
            type="button"
            onClick={onEnlarge}
            className="absolute bottom-6 right-6 inline-flex h-11 items-center gap-2 rounded-full border border-brass/30 bg-pearl/95 px-4 text-sm font-black text-garden shadow-soft transition hover:text-wine"
          >
        ...
        <div className="grid content-start gap-5 p-5 md:p-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-clay">{product.category}</p>
              <h2 className="mt-2 font-display text-4xl leading-tight text-merlot">{product.name}</h2>
            </div>
            <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-md border border-brass/25">
              <X size={18} />
            </button>
          </div>
          <div className="flex items-baseline gap-3">
            <strong className="text-2xl">{fmt(productPrice(product))}</strong>
            {product.salePrice && <del className="text-charcoal/45">{fmt(product.price)}</del>}
          </div>
          <p className="text-charcoal/70">{product.description}</p>
          <div className="grid gap-3 rounded-md border border-brass/25 bg-paper p-4 text-sm">
          ...
          <label className="grid gap-2 text-sm font-bold text-charcoal/70">
            ...
            <select
              ...
              className="h-11 rounded-md border border-brass/25 px-3"
            >
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
            <button
              ...
              disabled={!variantId}
              className="h-12 rounded-md bg-wine px-5 font-bold text-white transition hover:bg-merlot"
            >
              Add to cart
            </button>
            <button
              type="button"
              onClick={onWish}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-md border border-brass/35 bg-paper px-5 font-bold text-garden"
            >
          ...
          <div className="border-t border-brass/20 pt-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <h3 className="font-display text-2xl text-merlot">Catalogue view</h3>
              <button
                type="button"
                onClick={onEnlarge}
                className="inline-flex items-center gap-2 text-sm font-bold text-garden transition hover:text-wine"
              >
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
              {moreProducts.map((item) => (
                <button
                  ...
                  className="group overflow-hidden rounded-md border border-brass/25 bg-cream p-1 text-left transition hover:border-wine"
                  title={item.name}
                >
                  <img
                    src={item.image}
                  ...
                  />
                  <span className="mt-1 block truncate px-1 text-xs font-bold text-charcoal/70">
                    {item.name}
                  </span>
                </button>
              ))}
...
function ImageCatalogueViewer({ product, products, onClose, onSelect, onViewProduct }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="grid max-h-[calc(100vh-2rem)] w-[min(1180px,calc(100vw-2rem))] overflow-hidden rounded-md border border-brass/30 bg-ink text-white shadow-soft lg:grid-cols-[1fr_310px]">
        <div className="relative grid min-h-[68vh] place-items-center bg-black">
          ...
          <button
            ...
            className="absolute left-4 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-pearl/90 text-ink shadow-soft transition hover:bg-white"
          ...
          <button
            ...
            className="absolute right-4 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-pearl/90 text-ink shadow-soft transition hover:bg-white"
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full bg-pearl/90 text-ink shadow-soft transition hover:bg-white"
        ...
        <aside className="grid max-h-[78vh] content-start gap-4 overflow-auto bg-pearl p-5 text-ink">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-clay">Catalogue image</p>
            <h2 className="mt-2 font-display text-3xl text-merlot">{product.name}</h2>
          ...
          <button
            ...
            className="h-11 rounded-md bg-wine px-4 text-sm font-bold text-white transition hover:bg-merlot"
          >
            View product details
          </button>
          <div>
            <h3 className="mb-3 font-display text-2xl text-merlot">More products</h3>
            <div className="grid grid-cols-2 gap-2">
              {products.map((item) => (
                <button
                  key={item.id}
                  ...
                  className={classNames(
                    "overflow-hidden rounded-md border bg-cream p-1 text-left transition",
                    item.id === product.id ? "border-wine" : "border-brass/25 hover:border-wine",
                  )}
                >
...
function CartDrawer({ open, cart, summary, couponCode, fmt, onClose, onQuantity, onCoupon, onCheckout }) {
  ...
  return (
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 grid h-screen w-[min(430px,100vw)] grid-rows-[auto_1fr_auto] bg-pearl p-5 shadow-soft">
        <div className="flex items-center justify-between border-b border-brass/25 pb-4">
          <h2 className="font-display text-3xl text-merlot">Cart</h2>
          <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-md border border-brass/25">
            <X size={18} />
          </button>
        </div>
        <div className="overflow-auto py-4">
          {cart.length ? (
            <div className="grid gap-4">
              {cart.map((line) => (
                <div key={line.id} className="grid grid-cols-[76px_1fr] gap-3 border-b border-brass/20 pb-4">
                  ...
                  <div className="grid gap-1">
                    ...
                    {line.quantity > 1 && <span className="text-xs text-charcoal/45">{fmt(line.price)} each</span>}
                    <div className="mt-2 flex items-center gap-2">
                      <button type="button" onClick={() => onQuantity(line.id, -1)} className="grid h-8 w-8 place-items-center rounded border border-brass/25 transition hover:border-wine hover:text-wine">
                        <Minus size={14} />
                      </button>
                      ...
                      <button
                        ...
                        className="ml-2 grid h-8 w-8 place-items-center rounded border border-brass/25 text-charcoal/40 transition hover:border-wine hover:text-wine"
        ...
        <div className="border-t border-brass/25 pt-4">
          ...
          <button
            ...
            onClick={onCheckout}
            className="mt-4 h-12 w-full rounded-md bg-wine font-bold text-white transition hover:bg-merlot"
          >
            Checkout
          </button>
        </div>
...
function CheckoutModal({ cart, summary, settings, fmt, customer, onClose, onSubmit }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="max-h-[calc(100vh-2rem)] w-[min(980px,calc(100vw-2rem))] overflow-auto rounded-md border border-brass/30 bg-pearl p-5 shadow-soft md:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-clay">Secure checkout</p>
            <h2 className="mt-2 font-display text-4xl text-merlot">
            ...
            <div key={label} className="text-center">
              <span
                className={classNames(
                  ...
                  index + 1 <= step ? "bg-wine" : "bg-brass/20",
                )}
              />
              <span className={classNames("mt-1 block text-xs font-bold", index + 1 <= step ? "text-wine" : "text-charcoal/40")}>
                {label}
              </span>
            </div>
          ))}
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <div>
            {step === 1 && (
              <form onSubmit={handleDetailsSubmit} className="grid gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="grid gap-2 text-sm font-bold text-charcoal/70">
                    Customer type
                    <select name="accountType" className="h-11 rounded-md border border-brass/25 px-3">
                  ...
                  <label className="grid gap-2 text-sm font-bold text-charcoal/70">
                    Delivery option
                    <select name="delivery" className="h-11 rounded-md border border-brass/25 px-3">
                      {deliveryOptionsList.map((opt) => (
                        <option key={opt}>{opt}</option>
                      ))}
                    </select>
                  </label>
                  <label className="grid gap-2 text-sm font-bold text-charcoal/70 sm:col-span-2">
                    Shipping address
                    <input name="address" required defaultValue={c.address || ""} placeholder="Street, suburb, town, country" className="h-11 rounded-md border border-brass/25 px-3" />
                  </label>
                  <label className="grid gap-2 text-sm font-bold text-charcoal/70 sm:col-span-2">
                    Payment method
                    <select
                  ...
                  <label className="flex items-center gap-2 text-sm font-bold text-charcoal/70 sm:col-span-2">
                    <input name="marketingOptIn" type="checkbox" className="h-4 w-4" />
                    Receive order updates and follow-up messages
                  </label>
                </div>
                <button type="submit" className="h-12 rounded-md bg-wine font-bold text-white transition hover:bg-merlot">
                  Continue to payment
                </button>
              </form>
            )}
            ...
            {step === 2 && (
              <div className="grid gap-4">
                {isOnlinePayment && hasGateway && (
                  <div className="rounded-md border border-brass/25 bg-paper p-5">
                    <div className="mb-3 flex items-center gap-2">
                      <CreditCard size={20} className="text-wine" />
                      <h3 className="font-bold text-merlot">Card payment</h3>
                      <span className="ml-auto rounded-full bg-brass/10 px-2 py-0.5 text-[10px] font-black uppercase text-charcoal/50">
                    ...
                    <div className="grid gap-3">
                      ...
                      <label className="grid gap-2 text-sm font-bold text-charcoal/70">
                        ...
                        <input
                          ...
                          className="h-11 rounded-md border border-brass/25 px-3 font-mono tracking-wider"
                          required
                        />
                      </label>
                      <div className="grid grid-cols-2 gap-3">
                        <label className="grid gap-2 text-sm font-bold text-charcoal/70">
                          ...
                          <input
                            ...
                            placeholder="MM/YY"
                            className="h-11 rounded-md border border-brass/25 px-3 font-mono"
                            required
                          />
                        </label>
                        <label className="grid gap-2 text-sm font-bold text-charcoal/70">
                    ...
                    {settings.gatewayMode === "sandbox" && (
                      <p className="mt-3 rounded-md bg-marigold/10 px-3 py-2 text-xs text-charcoal/60">
                        <strong className="text-charcoal">Test mode:</strong> Use card number <span className="font-mono">4242 4242 4242 4242</span>, any future expiry, and any 3-digit CVC.
                      </p>
                    )}
                    <button
                      type="button"
                      ...
                      className="mt-4 h-12 w-full rounded-md bg-wine font-bold text-white transition hover:bg-merlot disabled:opacity-60"
                    >
                      {processing ? (
                ...
                {isOnlinePayment && !hasGateway && (
                  <div className="rounded-md border border-wine/20 bg-petal p-5">
                    <h3 className="font-bold text-wine">Online payment unavailable</h3>
                    ...
                    <button
                      type="button"
                      onClick={() => setStep(1)}
                      className="mt-3 h-10 rounded-md border border-brass/25 px-4 font-bold text-charcoal/70 transition hover:border-wine"
                    >
                      Go back
                    </button>
                  </div>
                ...
                {!isOnlinePayment && paymentMethod.toLowerCase().includes("eft") && (
                  <div className="rounded-md border border-brass/25 bg-paper p-5">
                    <h3 className="font-bold text-merlot">Bank / EFT transfer</h3>
                    <p className="mt-2 text-sm text-charcoal/70">
                    ...
                    <div className="mt-3 rounded-md border border-brass/20 bg-white p-3 text-sm">
                      ...
                      {settings.bankReference && <p className="mt-2 text-xs text-charcoal/55">{settings.bankReference}</p>}
                    </div>
                    <button
                      type="button"
                      onClick={() => setStep(3)}
                ...
                {!isOnlinePayment && paymentMethod.toLowerCase().includes("wallet") && (
                  <div className="rounded-md border border-brass/25 bg-paper p-5">
                    <h3 className="font-bold text-merlot">EWallet payment</h3>
                    <p className="mt-2 text-sm text-charcoal/70">
                    ...
                    <div className="mt-3 rounded-md border border-brass/20 bg-white p-3 text-sm">
                      ...
                      <InfoRow label="Reference" value="Your order number (shown after placing)" />
                      {settings.ewalletInstructions && <p className="mt-2 text-xs text-charcoal/55">{settings.ewalletInstructions}</p>}
                    </div>
                    <button
                      type="button"
                      onClick={() => setStep(3)}
                ...
                {!isOnlinePayment && paymentMethod.toLowerCase().includes("delivery") && (
                  <div className="rounded-md border border-brass/25 bg-paper p-5">
                    <h3 className="font-bold text-merlot">Pay upon delivery</h3>
                    <p className="mt-2 text-sm text-charcoal/70">
                ...
                {!isOnlinePayment && !paymentMethod.toLowerCase().includes("eft") && !paymentMethod.toLowerCase().includes("wallet") && !paymentMethod.toLowerCase().includes("delivery") && (
                  <div className="rounded-md border border-brass/25 bg-paper p-5">
                    <h3 className="font-bold text-merlot">{paymentMethod}</h3>
                    <p className="mt-2 text-sm text-charcoal/70">
                ...
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="h-10 rounded-md border border-brass/25 font-bold text-charcoal/60 transition hover:border-wine hover:text-wine"
                >
                  ← Back to details
                </button>
              </div>
            ...
            {step === 3 && (
              <div className="grid gap-4">
                <div className="rounded-md border border-brass/25 bg-paper p-5">
                  <h3 className="font-bold text-merlot">Review your order</h3>
                  <div className="mt-3 grid gap-2 text-sm">
                    ...
                    {formData?.gatewayResult && (
                      <div className="flex items-center gap-2 rounded-md bg-garden/10 px-3 py-2 text-sm font-bold text-garden">
                        <Check size={16} />
                    ...
                    {!formData?.gatewayResult && !formData?.proofOfPayment && !formData?.payment?.toLowerCase().includes("delivery") && (
                      <div className="rounded-md bg-petal px-3 py-2 text-sm font-bold text-wine">
                ...
                <button
                  ...
                  className="h-12 rounded-md bg-wine font-bold text-white transition hover:bg-merlot"
                >
          ...
          <div className="rounded-md border border-brass/25 bg-paper p-4">
            <h3 className="font-display text-2xl text-merlot">Order review</h3>
...
function OrderDetailModal({ order, settings, fmt, onClose }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="max-h-[calc(100vh-2rem)] w-[min(920px,calc(100vw-2rem))] overflow-auto rounded-md border border-brass/30 bg-pearl p-5 shadow-soft md:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-clay">Order confirmation</p>
            <h2 className="mt-2 font-display text-4xl text-merlot">{order.orderNumber}</h2>
          </div>
          <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-md border border-brass/25">
            <X size={18} />
          </button>
        ...
        <div className="mt-4 flex items-center gap-3">
          <span
            className={classNames(
              ...
              isPaid ? "bg-garden/10 text-garden" : "bg-petal text-wine",
            )}
          >
            {order.paymentStatus}
          </span>
          <span className="rounded-full bg-brass/10 px-3 py-1 text-sm font-bold text-charcoal/70">
            {order.status}
          </span>
          <span className="text-sm text-charcoal/50">{dateStr}</span>
        </div>
        ...
        <div className="mt-3 grid grid-cols-5 gap-1">
          {steps.map((step, index) => (
            <div key={step} className="text-center">
              <span
                className={classNames("block h-2 rounded-full", index <= activeIndex ? "bg-garden" : "bg-brass/20")}
              />
              <span className={classNames("mt-1 block text-[10px]", index <= activeIndex ? "font-bold text-garden" : "text-charcoal/40")}>
                {step}
              </span>
            </div>
          ))}
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <div className="grid gap-5">
            <div className="rounded-md border border-brass/25 bg-paper p-4">
              <h3 className="font-bold text-merlot">Order items</h3>
              <div className="mt-3 grid gap-3">
                {(order.items || []).map((item) => (
                  <div key={item.id} className="flex items-center gap-3 border-b border-brass/15 pb-3 last:border-b-0 last:pb-0">
                    <img src={item.image} alt="" className="h-16 w-12 rounded object-cover" />
                    <div className="flex-1">
                      <strong className="block text-sm">{item.name}</strong>
                      <span className="text-xs text-charcoal/60">{item.size} / {item.color} × {item.quantity}</span>
                    </div>
            ...
            {!isPaid && (
              <div className="rounded-md border border-wine/20 bg-petal p-4">
                <h3 className="font-bold text-wine">Payment required</h3>
                ...
                {order.payment === "EFT bank transfer" || (proofMethods.includes(order.payment) && order.payment.toLowerCase().includes("eft")) ? (
                  <div className="mt-3 rounded-md border border-brass/25 bg-white p-3 text-sm">
                    ...
                    <div className="mt-2 grid gap-1">
                      ...
                      {settings.bankReference && (
                        <p className="mt-2 text-xs text-charcoal/60">{settings.bankReference}</p>
                      )}
                    </div>
                  </div>
                ) : null}
                  ...
                  <div className="mt-3 rounded-md border border-brass/25 bg-white p-3 text-sm">
                    ...
                    <div className="mt-2 grid gap-1">
                      ...
                      {settings.ewalletInstructions && (
                        <p className="mt-2 text-xs text-charcoal/60">{settings.ewalletInstructions}</p>
                      )}
                    </div>
                  </div>
                ) : null}
                ...
                {order.proofOfPayment && (
                  <div className="mt-3 flex items-center gap-2 rounded-md bg-garden/10 px-3 py-2 text-sm font-bold text-garden">
                    <Check size={16} />
            ...
            {isPaid && (
              <div className="flex items-center gap-2 rounded-md bg-garden/10 px-4 py-3 text-sm font-bold text-garden">
                <Check size={18} />
          ...
          <div className="grid content-start gap-4">
            <div className="rounded-md border border-brass/25 bg-paper p-4">
              <h3 className="font-bold text-merlot">Order summary</h3>
              <div className="mt-3 grid gap-2 text-sm">
            ...
            <div className="rounded-md border border-brass/25 bg-paper p-4">
              <h3 className="font-bold text-merlot">Details</h3>
              <div className="mt-3 grid gap-2 text-sm">
            ...
            {settings.whatsappNumber && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-garden px-4 font-bold text-white transition hover:bg-garden/90"
              >
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-md border border-brass/25 px-4 font-bold text-charcoal/70 transition hover:border-wine hover:text-wine"
            >
              Close
            </button>
          </div>
...
function AdminPanel({
  ...
  const threshold = Number(store.settings?.lowStockThreshold ?? 2);
    ...
    <div className="min-h-[560px] rounded-md border border-garden/20 bg-pearl p-5 shadow-soft">
        ...
        <div className="grid gap-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="inline-flex items-center gap-2 rounded-full bg-garden/10 px-3 py-1 text-sm font-bold text-garden">
              <ShieldCheck size={16} /> Role permissions active
            </span>
          </div>
          <MetricGrid
            metrics={[
              ...
              ["Wishlist saves", store.wishlist.length, Heart],
            ]}
          />
        </div>
      )}
          ...
          <form onSubmit={onProductAdd} className="grid gap-3 rounded-md border border-brass/25 bg-paper p-4">
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <label className="grid gap-2 text-sm font-bold text-charcoal/70 md:col-span-2">
                Image URL
                <input name="image" className="h-11 rounded-md border border-brass/25 bg-white px-3" />
              </label>
      ...
      {activeTab === "content" && (
        <div className="grid gap-5">
          ...
          <form onSubmit={onContentSave} className="grid gap-4">
            <TextInput name="announcement" label="Announcement bar" defaultValue={store.content.announcement} />
            <TextInput name="heroTitle" label="Hero title" defaultValue={store.content.heroTitle} />
            <TextInput name="heroImage" label="Hero image URL" defaultValue={store.content.heroImage} />
            <label className="grid gap-2 text-sm font-bold text-charcoal/70">
              Hero subtitle
              <textarea name="heroSubtitle" defaultValue={store.content.heroSubtitle} className="min-h-24 rounded-md border border-brass/25 p-3" />
            </label>
            <TextInput name="campaignTitle" label="Collection section title" defaultValue={store.content.campaignTitle} />
            <label className="grid gap-2 text-sm font-bold text-charcoal/70">
              Collection section copy
              <textarea name="campaignCopy" defaultValue={store.content.campaignCopy} className="min-h-24 rounded-md border border-brass/25 p-3" />
            </label>
            <button className="inline-flex h-11 w-fit items-center gap-2 rounded-md bg-wine px-4 font-bold text-white">
              <Check size={18} /> Save content
            </button>
          </form>
        </div>
      )}
            ...
            rows={
              store.orders.length
                ? store.orders.map((order) => [
                    ...
                    >
                      {splitLines(store.settings?.orderStatuses).map((status) => (
      ...
      {activeTab === "promotions" && (
        <div className="grid gap-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <PromoCard code="SIRKA10" title="10% off" copy="Applies to carts over N$800." />
            <PromoCard code="FREESHIP" title="Free delivery" copy="Removes delivery fee at checkout." />
          </div>
          <div className="rounded-md border border-brass/25 bg-paper p-4">
            <h4 className="font-bold text-merlot">Seasonal promotion planning</h4>
            <p className="mt-1 text-sm text-charcoal/65">
              Plan codes by collection, calendar moment, minimum spend, and delivery offer.
            </p>
          </div>
        </div>
      )}
...
function OrderMini({ order, fmt, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-md border border-brass/25 bg-paper p-4 text-left transition hover:border-wine hover:shadow-sm"
    >
      ...
      <div className="mt-1 flex items-center justify-between gap-2 text-sm">
        <span className="text-charcoal/60">{dateStr}</span>
        <span
          className={classNames(
      ...
      <div className="mt-3 grid grid-cols-5 gap-1">
        {steps.map((step, index) => (
          <span
            key={step}
            title={step}
            className={classNames("h-2 rounded-full", index <= activeIndex ? "bg-garden" : "bg-brass/20")}
          />
        ))}
      </div>
    </button>
...
function CustomerAccountView({ store, customerEmail, fmt, onViewOrder, onLogout, onSelectProduct }) {
  ...
  return (
    <div className="mt-8 grid gap-5 lg:grid-cols-[.85fr_1.15fr_1fr]">
      <div className="rounded-md border border-brass/25 bg-pearl p-5 shadow-sm">
        <User className="mb-4 text-wine" size={24} />
        <h3 className="font-display text-2xl text-merlot">My account</h3>
        <div className="mt-3 grid gap-2 text-sm">
        ...
        <button
          type="button"
          onClick={onLogout}
          className="mt-5 flex h-10 w-full items-center justify-center gap-2 rounded-md border border-brass/25 text-sm font-bold text-charcoal/60 transition hover:border-wine hover:text-wine"
        >
      ...
      <div className="rounded-md border border-brass/25 bg-pearl p-5 shadow-sm">
        <h3 className="font-display text-2xl text-merlot">My orders</h3>
        <div className="mt-4 grid gap-3 max-h-[460px] overflow-y-auto pr-1">
          {myOrders.length ? (
            ...
            <div className="rounded-md border border-brass/20 bg-paper p-4 text-center text-sm text-charcoal/50">
      ...
      <div className="rounded-md border border-brass/25 bg-pearl p-5 shadow-sm">
        <h3 className="font-display text-2xl text-merlot">Wishlist</h3>
        <div className="mt-4 grid gap-3">
            ...
            store.wishlist
              ...
              .map((product) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-md border border-brass/25 bg-white p-2 text-left transition hover:border-wine"
                >
                  <img src={product.image} alt="" className="h-16 w-12 rounded object-cover" />
                  <span>
                    <strong className="block">{product.name}</strong>
                    <span className="text-sm text-charcoal/60">{fmt(productPrice(product))}</span>
                  </span>
                </button>
              ))
          ) : (
...
function SummaryRows({ summary, fmt }) {
  return (
    <div className="mt-4 grid gap-2 text-sm">
      ...
      <div className="flex items-center justify-between gap-4 border-t border-brass/25 pt-3 text-lg">
